...
int main(int argc, char *argv[])
{
...

    {
        ...
        /* root allocates global arrays and initialises them */
        if (proc == 0) {
            ...
            /* root copies its own interior block */
            {
            ...
            /* root sends interior blocks to all other ranks */
            {
                int dest;
            ...
            /* non-root receives its interior block */
        ...
        /* fill physical boundary ghost rows */
        if (proc == 0) {
        ...
        /* halo exchange for internal boundaries */
        ...
        /* gather rhs_local back to root as rhs_global */
        {
            int count_local = (imax + 2) * local_jmax;
        ...
        /* ---------------------------------------------------------
        Transitional pressure solve:
        root solves global poisson using rhs_global and p_global
        --------------------------------------------------------- */
        ...
        /* ---------------------------------------------------------
        Distribute p_global back to each rank's p_local interior
        --------------------------------------------------------- */
        {
            int count_local = (imax + 2) * local_jmax;

            if (proc == 0) {
                /* root copies its own p block */
                ...
                /* send p blocks to all other ranks */
        ...
        /* refresh p ghost rows after redistribution */
        if (proc == 0) {
            float *top_p = (float *)malloc((imax + 2) * sizeof(float));
            if (!top_p) {
    ...
    if (proc == 0) {
        ...
        /* ---------------------------------------------------------
        Gather final local u/v/p/flag back to global arrays on root
        --------------------------------------------------------- */
        {
            int count_local = (imax + 2) * local_jmax;
            ...
            if (proc == 0) {
                /* root copies its own local data back */
                ...
                /* restore physical boundary rows in global arrays from original edge data */
                /* top row j=0 and bottom row j=jmax+1 were never gathered from interiors */
                /* they already exist in root-owned global arrays and remain valid */
            } else {
