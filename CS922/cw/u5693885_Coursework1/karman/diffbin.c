...
static inline float rel_diff(float a, float b, float floor_val)
{
...
int main(int argc, char **argv)
{
    ...
    float rel_floor = 1e-30f;     /* prevents divide-by-zero */
    int mode = MODE_DIFF;
    ...
    for (i = 0; i < imax + 2 && !diff_found; i++) {
        ...
        for (j = 0; j < jmax + 2 && !diff_found; j++) {
            ...
            switch (mode) {
                case MODE_DIFF: /* NaN and infinity checks */
                    ...
                    break;
                    /* Relative tolerance check (only for u,v,p) */
                    if (rel_epsilon > 0.0f) {
                        float ru = rel_diff(u1[j], u2[j], rel_floor);
                        float rv = rel_diff(v1[j], v2[j], rel_floor);
                        float rp = rel_diff(p1[j], p2[j], rel_floor);

                        if (ru > rel_epsilon || rv > rel_epsilon || rp > rel_epsilon) {
                            diff_found = 1;
                            break;
                        }
                    }
...
static void print_help(void)
{
    ...
    fprintf(stderr, "  -r, --rel-epsilon=RELEPS Set relative tolerance (0 disables)\n");
